window.CD = {
  ...
  getProfile: function(company, user, type, callback) {
    var profileEndpoint = "http://profile.movableink.com:8081/";

    CD.getCORS(profileEndpoint + 'company/' + company + "/profile/" + user,  function(profileData) {
      var productArray = [];
      var parsedEvents = JSON.parse(profileData);

      switch(type) {
        case "cart_abandon":
          var productMap = {};
          parsedEvents.forEach(function(userEvent) {
            if (userEvent.event_type === "cart_add") {
              productMap[userEvent.product_url] = "in_cart";
            } else if (userEvent.event_type === "conversion") {
              productMap[userEvent.product_url] = "purchased";
            }
          });
          for(var product_url in productMap) {
            if (productMap[product_url] === 'in_cart') {
              productArray.push(product_url);
            }
          }
          break;
        default:
